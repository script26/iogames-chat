window.onload = function () {
    ...
    let userName = getCookie("nme");
    if (!userName) {
        // Generate a random color if no cookie exists
		let userName = "Guest" + getRandomDigit();
    ...
    if (window["WebSocket"]) {
        ...
        conn.onclose = function (evt) {
			// Create a new div element to display the connection closed message
            var item = document.createElement("div");
            item.innerHTML = "<b>Connection closed.</b>"; // Add a bold message to the div
            appendLog(item); // Append the message to the log (chat window or message log)
        };
